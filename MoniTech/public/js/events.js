const switchDropDown = function (){
  ...
  const img = document.querySelector('.drop-down')
  ...
  dashboard.value = dashboard.value? false: true;
}

function load() {
  const click = document.querySelectorAll('.click-dashboard')
  click.forEach(obj => {
  ...
  fetch(`./setups/detalhes/${id}`).then(function(resposta){
    if (resposta.ok) {
        resposta.json().then(setup=>{
          ...
          document.getElementById('ram').innerHTML = `Memória RAM: ${setup.memoriaRam}`;
